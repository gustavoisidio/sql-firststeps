...
CREATE TABLE salarios (
    ...
    codigo_grau NUMBER NOT NULL,
	data_titulacao DATE NOT NULL,
    CONSTRAINT salarios_pk PRIMARY KEY (rg),
    CONSTRAINT salarios_FK1 FOREIGN KEY (cpf, data_titulacao) REFERENCES titulacao_empregado (cpf, data_titulacao)
);

-- 80. Uso de NEW e OLD em TRIGGER de atualização
-- Quando há uma redução no salario de alguém, o trigger guarda isso na tabela trigger_ReducaoSalarial
DROP TABLE reducaoS;
CREATE TABLE reducaoS (
    cpf NUMBER NOT NULL,
    CONSTRAINT reducaoS_pk PRIMARY KEY (cpf)
);
...
BEGIN
    INSERT INTO reducaoS (cpf) VALUES(:NEW.cpf);
END;

-- 74. AFTER TRIGGER
-- Sempre que uma mulher receber um aumento, guarda cpf dela na tabela aumentoMulheres
DROP TABLE aumentoMulheres;
CREATE TABLE aumentoMulheres (
    cpf NUMBER NOT NULL,
    CONSTRAINT aumentoMulheres_pk PRIMARY KEY (cpf)
);
