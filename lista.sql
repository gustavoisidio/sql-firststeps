...
WHERE e.cpf = g.cpf AND e.salario <= 4000 AND e.salario >= 3000;
...
WHERE e.salario <= ((
    ...
    FROM empregado e ) - 1000) AND e.salario >= ((SELECT MIN(e.salario)
                                                FROM empregado e ) - 1000);

-- Q3
SELECT DISTINCT g.cpf
FROM graduado g, participa a, projeto p
WHERE a.codigo_projeto = p.codigo AND p.valor > (SELECT AVG(p.valor)
                                                FROM projeto p);
